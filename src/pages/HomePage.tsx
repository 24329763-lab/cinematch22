...
import {
  Sparkles, Heart, Star, Compass, Flame, TrendingUp, Clock, Globe, Loader2, MessageCircle, Users,
} from "lucide-react";
...
const ICON_MAP: Record<string, React.ElementType> = {
  heart: Heart, flame: Flame, compass: Compass, star: Star,
  trending: TrendingUp, clock: Clock, globe: Globe, sparkles: Sparkles,
};
...
const ChatCTA = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mx-5 mt-10 mb-4 glass-surface rounded-2xl p-6 text-center">
      ...
      <h3 className="text-lg font-bold text-foreground mb-2">Melhore sua home conversando sobre seus gostos</h3>
      <p className="text-sm text-muted-foreground leading-relaxed mb-4 max-w-sm mx-auto">
        Fale comigo sobre seus filmes favoritos — quanto mais eu souber, melhores serão as recomendações.
      </p>
      <button onClick={() => navigate("/chat")} className="gradient-primary text-primary-foreground px-6 py-3 rounded-full text-sm font-bold cinema-glow-sm hover:opacity-90 transition-opacity">
        <span className="flex items-center gap-2"><Sparkles size={14} /> Conversar agora</span>
      </button>
    </motion.div>
  );
};

interface FriendWatchlist { friendName: string; friendId: string; movies: MoviePoster[] }

const HomePage = () => {
  const { user } = useAuth();
  const { personalizedSections, tasteSummary, isLoading: personalizationLoading, hasPersonalization } = usePersonalizedHome();
  const { sections: tmdbSections, isLoading: tmdbLoading } = useTMDBMovies();
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("watchlist").select("*").eq("user_id", user.id).order("added_at", { ascending: false }).limit(20)
      .then(({ data }) => {
        if (data && data.length > 0) {
          setWatchlistItems(data.map((item: any) => ({
            id: item.movie_id, title: item.title, year: item.year || 2024, rating: item.rating || 7.5,
            posterUrl: item.poster_url || "/placeholder.svg",
            platforms: (item.platforms || []) as ("netflix" | "prime" | "disney")[],
            genres: item.genres || [], description: "",
          })));
        }
      });
  }, [user]);
  ...
  useEffect(() => {
    ...
    const loadFriendRows = async () => {
      const { data: invites } = await supabase.from("friend_invites").select("*").eq("status", "accepted").or(`sender_id.eq.${user.id},receiver_id.eq.${user.id}`);
      ...
      const [profilesRes, ...watchlistResults] = await Promise.all([
        ...
        ...friendIds.map((fid: string) => supabase.from("watchlist").select("movie_id, title, year, rating, poster_url, platforms, genres").eq("user_id", fid).order("added_at", { ascending: false }).limit(15)),
      ]);
  ...
  // Determine which sections to show as fallback
  const showFallback = !hasPersonalization && !personalizationLoading;
  const fallbackSections = tmdbSections.length > 0 ? tmdbSections : [];
  ...
  return (
    <div className="min-h-[calc(100dvh-4rem)] overflow-y-auto pb-24 relative">
      ...
      {tasteSummary && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mx-5 mt-8 mb-2 glass rounded-2xl p-4 flex items-start gap-3">
      ...
      {/* TMDB Fallback sections */}
      {showFallback && fallbackSections.map((section) => {
        const IconComp = ICON_MAP[section.icon] || Star;
        return (
      ...
      {/* Loading TMDB fallback */}
      {showFallback && tmdbLoading && (
        <div className="flex items-center justify-center gap-2 mt-8 text-muted-foreground">
          <Loader2 size={16} className="animate-spin" />
          <span className="text-xs">Carregando filmes...</span>
        </div>
      )}

      {/* Chat CTA */}
      {user && !personalizationLoading && (!hasPersonalization || personalizedSections.length < 3) && <ChatCTA />}
      {!user && !hasPersonalization && <ChatCTA />}
